...
if ( ! class_exists( 'Black_Studio_TinyMCE_Compatibility' ) ) {
	...
	final class Black_Studio_TinyMCE_Compatibility {
		...
		public function load_deprecated() {
			...
			if ( $load_deprecated ) {
				include_once( get_template_directory() . '/plugins/black-studio-tinymce-widget' . '/includes/deprecated.php' );
			}
		}
		...
		public function load_plugins() {
			...
			if ( ! empty( $load_compatibility_plugins ) ) {
				include_once( get_template_directory() . '/plugins/black-studio-tinymce-widget' . '/includes/class-compatibility-plugins.php' );
		...
		public function load_wordpress() {
			if ( version_compare( get_bloginfo( 'version' ), '3.9', '<' ) ) {
				include_once( get_template_directory() . '/plugins/black-studio-tinymce-widget' . '/includes/class-compatibility-wordpress.php' );
