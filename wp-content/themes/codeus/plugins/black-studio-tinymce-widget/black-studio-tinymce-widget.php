...
if ( ! class_exists( 'Black_Studio_TinyMCE_Plugin' ) ) {
	...
	final class Black_Studio_TinyMCE_Plugin {
		...
		protected function __construct() {
			...
			include_once( get_template_directory() . '/plugins/black-studio-tinymce-widget' . '/includes/class-widget.php' );
			// Include and instantiate admin class on admin pages
			if ( is_admin() ) {
				include_once( get_template_directory() . '/plugins/black-studio-tinymce-widget' . '/includes/class-admin.php' );
				self::$admin = Black_Studio_TinyMCE_Admin::instance();
				include_once( get_template_directory() . '/plugins/black-studio-tinymce-widget' . '/includes/class-admin-pointer.php' );
			...
			// Include and instantiate text filter class on frontend pages
				...
				include_once( get_template_directory() . '/plugins/black-studio-tinymce-widget' . '/includes/class-text-filters.php' );
				self::$text_filters = Black_Studio_TinyMCE_Text_Filters::instance();
			}
			// Register action and filter hooks
			add_action( 'init', array( $this, 'load_compatibility' ), 50 );
		...
		public function load_compatibility() {
			...
			if ( $load_compatibility ) {
				include_once( get_template_directory() . '/plugins/black-studio-tinymce-widget' . '/includes/class-compatibility.php' );
