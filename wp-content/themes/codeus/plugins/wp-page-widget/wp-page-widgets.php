...
function pw_print_scripts() {
	...
	if (
		...
		wp_enqueue_script('pw-widgets', get_template_directory_uri() . '/plugins/wp-page-widget/' . 'assets/js/page-widgets.js', array('jquery', 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-droppable'), PAGE_WIDGET_VERSION, true);
...
function pw_print_styles() {
	...
	if (
		...
		if ( version_compare( get_bloginfo('version'), '3.8', '<' ) ) {
			wp_enqueue_style('pw-widgets', get_template_directory_uri() . '/plugins/wp-page-widget/' . 'assets/css/page-widgets.css', array(), PAGE_WIDGET_VERSION);
		}
	}

	if ( version_compare( get_bloginfo('version'), '3.8', '<' ) ) {
		wp_enqueue_style('pw-style', get_template_directory_uri() . '/plugins/wp-page-widget/' . 'assets/css/style.css', array(), PAGE_WIDGET_VERSION);
	} else {
		wp_enqueue_style('pw-style', get_template_directory_uri() . '/plugins/wp-page-widget/' . 'assets/css/style-3.8.css', array(), PAGE_WIDGET_VERSION);
	}
...
function pw_admin_menu() {
	...
	if (current_user_can('edit_posts')) {
		...
		foreach ($settings['post_types'] as $post_type) {
			add_meta_box('pw-widgets', 'Page Sidebar', 'pw_metabox_content', $post_type, 'normal', 'high');
		}
...
function pw_get_settings() {
	$defaults = array(
		'post_types' => array('page', 'codeus_portfolios'),
		'taxonomies' => array(),
		'donation' => 'yes',
	...
	$settings = false; //get_option('pw_options', array());
