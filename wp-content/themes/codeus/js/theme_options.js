(function($) {
	$(document).ready(function() {
		...
		function codeus_updateAdminFonts(root) {
			var fonts = [];
		...
		$('.font-select select').each(function() {
			...
			$this.change(function(e) {
				codeus_updateAdminFonts($(this).closest('.font-select'));
			});
		});

		codeus_updateAdminFonts($('.font-select'));
		...
		var skins_defaults = JSON.parse(theme_options_object.codeus_color_skin_defaults);
		$('#page_color_style:input').change(function() {
			if(confirm('Change colors, backgrounds and fonts options?')){
				for(item in skins_defaults) {
					$('#'+item+':input').val(skins_defaults[item][$(this).val()]);
